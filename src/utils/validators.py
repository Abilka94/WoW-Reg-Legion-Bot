...
def check_password_strength(pwd: str) -> tuple[bool, str]:
    """
    Проверяет сложность пароля (простой/сложный)
    
    Пароль считается простым если:
    - Только буквы (без цифр и спецсимволов)
    - Только цифры
    - Меньше 10 символов и нет спецсимволов
    - Содержит только строчные или только заглавные буквы
    
    Args:
        pwd: Пароль для проверки
    
    Returns:
        Кортеж (is_strong, warning_message)
        is_strong: True если пароль сложный, False если простой
        warning_message: Сообщение-предупреждение (пустая строка если пароль сложный)
    """
    if not pwd or len(pwd) < 8:
        return True, ""  # Если пароль не валиден, не проверяем сложность
    ...
    # Проверка: только строчные или только заглавные буквы (без спецсимволов)
    ...
    # Проверка: нет спецсимволов и длина меньше 10
