...
async def main():
    ...
    @dp.message(Command("version"), F.chat.type == ChatType.PRIVATE)
    ...
    async def cb_back_main(callback: CallbackQuery, state: FSMContext):
        await callback.answer()  # Отвечаем сразу, чтобы кнопка не висела
        await state.clear()
    ...
    async def cb_my_account(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await callback.message.answer(text, reply_markup=kb_account_list(accounts) if accounts else kb_back())
    ...
    async def cb_select_account(callback: CallbackQuery, state: FSMContext):
        ...
        if not accounts:
            try:
                ...
                await callback.message.answer(T["account_no_account"], reply_markup=kb_back())
            return
        ...
        try:
            ...
            await callback.message.answer(text, reply_markup=kb_account_list(accounts, selected_email=email))
    ...
    async def cb_change_password(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await callback.message.answer(T["change_password_prompt"], reply_markup=kb_back())

    @dp.message(ChangePasswordStates.new_password, F.text)
        ...
        try:
            ...
            if success:
                try:
                    ...
                    await callback.message.answer(T["delete_account_success"], reply_markup=kb_back())
            else:
                try:
                    await callback.message.edit_text(T["delete_account_error"], reply_markup=kb_back())
                except TelegramBadRequest:
                    await callback.message.answer(T["delete_account_error"], reply_markup=kb_back())
        except Exception as e:
            ...
            try:
                ...
                await callback.message.answer("❌ Ошибка при удалении аккаунта", reply_markup=kb_back())
    ...
    async def cb_wiz_nav(callback: CallbackQuery, state: FSMContext):
        ...
        if current_state == RegistrationStates.nick.state:
            ...
            try:
                ...
                await callback.message.answer(text, reply_markup=kb_wizard(0))
            ...
            try:
                ...
                await callback.message.answer(text, reply_markup=kb_wizard(1))
        

    @dp.message(RegistrationStates.nick, F.text)
    ...
    @dp.message(RegistrationStates.pwd, F.text)
    ...
    @dp.message(RegistrationStates.mail, F.text)
    async def step_mail(message: Message, state: FSMContext):
        ...
        try:
            ...
            pass
        # Улучшенная валидация email с проверкой провайдера
        from src.utils.validators_enhanced import validate_email_enhanced
        from src.utils.email_providers import validate_email_provider
        
        email_result = validate_email_enhanced(email)
        if not email_result:
            error_msg = email_result.errors[0] if email_result.errors else T["err_mail"]
            await message.answer(f"❌ {error_msg}")
            return
        
        # Дополнительная проверка провайдера
        is_valid_provider, provider_error = validate_email_provider(email)
        if not is_valid_provider:
            await message.answer(f"❌ {provider_error}")
            return
    ...
    async def cb_admin_check_db(callback: CallbackQuery):
        ...
        try:
            ...
            await callback.message.answer(text, reply_markup=kb_admin_back())
    ...
    async def cb_admin_delete_account(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await callback.message.answer(T["admin_delete_prompt"], reply_markup=kb_admin_back())

    @dp.message(AdminStates.delete_account_input, F.text)
    async def step_admin_delete_account(message: Message, state: FSMContext):
        ...
        email_result = validate_email_enhanced(email)
        if not email_result:
            error_msg = email_result.errors[0] if email_result.errors else "Некорректный e-mail"
            await message.answer(f"❌ {error_msg}")
            return
    ...
    # Обработчик для групп - запрещаем работу в группах
    @dp.message(F.chat.type.in_([ChatType.GROUP, ChatType.SUPERGROUP]))
    async def handle_group_messages(message: Message):
        """Бот не работает в группах"""
        await message.answer("❌ Этот бот работает только в личных сообщениях. Пожалуйста, напишите боту в личку.")
        logger.info(f"Попытка использования бота в группе {message.chat.id} от пользователя {message.from_user.id}")
        return
    
    # Обработчик для каналов
    @dp.message(F.chat.type == ChatType.CHANNEL)
    async def handle_channel_messages(message: Message):
        """Бот не работает в каналах"""
        return
    
    # Обработчик только текстовых сообщений в личных чатах
    ...
    # Обработчик для всех не-текстовых сообщений (файлы, стикеры, опросы и т.д.)
    @dp.message(F.chat.type == ChatType.PRIVATE, ~F.text)
    async def handle_non_text_messages(message: Message, state: FSMContext):
        """Игнорируем файлы, стикеры, опросы и другие не-текстовые сообщения"""
        current_state = await state.get_state()
        
        # Пропускаем в состояниях FSM (на случай если пользователь отправил что-то не то)
        if current_state in (
            RegistrationStates.nick.state,
            RegistrationStates.pwd.state,
            RegistrationStates.mail.state,
        ...
        # Игнорируем не-текстовые сообщения вне состояний FSM
        await message.answer("❌ Бот принимает только текстовые сообщения. Используйте меню или команды.", reply_markup=kb_main(is_admin=message.from_user.id == ADMIN_ID))
        logger.info(f"Получено не-текстовое сообщение от пользователя {message.from_user.id}: тип {message.content_type}")
